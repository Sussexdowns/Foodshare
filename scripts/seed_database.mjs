/**
 * Foodshare Database Seed Script
 * 
 * This script seeds the Firebase Data Connect database with initial data
 * for the Foodshare platform. It uses the Firebase Data Connect REST API.
 * 
 * Usage:
 *   npm run seed          # Seed with default (production) project
 *   npm run seed:dev      # Seed the emulator
 *   npm run seed:prod     # Seed the production project
 * 
 * Environment Variables:
 *   FDC_PROJECT_ID  - Firebase project ID (default: foodshare-50695)
 *   FDC_LOCATION    - Data Connect location (default: us-east4)
 *   FDC_EMULATOR_HOST - Set to "localhost:8080" for emulator mode
 *   FDC_AUTH_TOKEN  - Firebase ID token (required for production)
 * 
 * Prerequisites:
 *   1. Authenticate with Firebase: firebase login
 *   2. For production: export FDC_AUTH_TOKEN=$(firebase auth:tokens:create --project=foodshare-50695 | jq -r '.idToken')
 *   3. For emulator: start the emulator: firebase emulators:start --only dataconnect
 */

import { createClient } from "@dataconnect/generated/react";
...
/** Execute a GraphQL mutation against Data Connect REST API */
...
/** Execute a transaction with multiple mutations */
async function executeTransaction(mutations) {
  const combinedDoc = mutations.map((m) => m.document).join("\n");
  const combinedVars = Object.assign({}, ...mutations.map((m) => m.variables));
  return executeMutation(combinedDoc, combinedVars);
}
...
async function seedCategories() {
  ...
  const data = categories.map(c => ({ ...c, createdAt: now }));
  const result = await executeMutation(MUTATION_CATEGORY_INSERT, { data });
  logSuccess(`Inserted ${result.category_insertMany?.length || data.length} categories`);
  return result;
}
...
async function seedUsers() {
  ...
  const now = new Date().toISOString();
  const data = users.map(u => ({ ...u, createdAt: now }));
  const result = await executeMutation(MUTATION_USER_INSERT, { data });
  logSuccess(`Inserted ${result.user_insertMany?.length || data.length} users`);
  return result;
}

async function seedFoodItems() {
  logStep("Seeding food items...");
  const items = generateFoodItems();
  const now = new Date().toISOString();
  const data = items.map(i => ({ ...i, createdAt: now }));
  const result = await executeMutation(MUTATION_FOODITEM_INSERT, { data });
  logSuccess(`Inserted ${result.foodItem_insertMany?.length || data.length} food items`);
  return result;
}
...
async function seedLocations() {
  ...
  const now = new Date().toISOString();
  const data = locations.map(l => ({ ...l, createdAt: now }));
  const result = await executeMutation(MUTATION_LOCATION_INSERT, { data });
  logSuccess(`Inserted ${result.location_insertMany?.length || data.length} locations`);
  return result;
}
...
async function seedRequests() {
  ...
  const now = new Date().toISOString();
  const data = requests.map(r => ({ ...r, createdAt: now }));
  const result = await executeMutation(MUTATION_REQUEST_INSERT, { data });
  logSuccess(`Inserted ${result.request_insertMany?.length || data.length} requests`);
  return result;
}
...
async function seedCommunities() {
  ...
  const now = new Date().toISOString();
  const data = communities.map(c => ({ ...c, createdAt: now }));
  const result = await executeMutation(MUTATION_COMMUNITY_INSERT, { data });
  logSuccess(`Inserted ${result.community_insertMany?.length || data.length} communities`);
  return result;
}
...
async function seedReviews() {
  ...
  const now = new Date().toISOString();
  const data = reviews.map(r => ({ ...r, createdAt: now }));
  const result = await executeMutation(MUTATION_REVIEW_INSERT, { data });
  logSuccess(`Inserted ${result.review_insertMany?.length || data.length} reviews`);
  return result;
}
...
async function seedReports() {
  ...
  const now = new Date().toISOString();
  const data = reports.map(r => ({ ...r, createdAt: now }));
  const result = await executeMutation(MUTATION_REPORT_INSERT, { data });
  logSuccess(`Inserted ${result.report_insertMany?.length || data.length} reports`);
  return result;
}
...
async function clearAllData() {
  ...
  const countResult = await executeMutation(MUTATION_CHECK);
  const counts = countResult[MUTATION_CHECK.match(/(\w+)\s*{/g)?.[0]?.replace(/[{\s]/g, '') || 'categories'];

  // Delete in dependency order
  const deleteMutations = [
    `mutation { report_deleteMany(all: true) }`,
    `mutation { review_deleteMany(all: true) }`,
    `mutation { request_deleteMany(all: true) }`,
    `mutation { community_deleteMany(all: true) }`,
    `mutation { foodItem_deleteMany(all: true) }`,
    `mutation { location_deleteMany(all: true) }`,
    `mutation { user_deleteMany(all: true) }`,
    `mutation { category_deleteMany(all: true) }`,
  ];
  ...
  for (const mutation of deleteMutations) {
    try {
      ...
      logWarning(`Delete mutation failed (may be expected if table is empty): ${e.message}`);
    }
  }

  logSuccess("All data cleared");
}
...
async function verifyData() {
  ...
  const data = result[MUTATION_CHECK.match(/(\w+)\s*{/g)?.[0]?.replace(/[{\s]/g, '') || 'categories'];
  const counts = result.categories ? result : data;

  console.log(`\n  ${BOLD}Table Counts:${RESET}`);
  console.log(`  ├─ Categories:  ${counts.categories._count}`);
  console.log(`  ├─ Users:       ${counts.users._count}`);
  console.log(`  ├─ Food Items:  ${counts.foodItems._count}`);
  console.log(`  ├─ Locations:   ${counts.locations._count}`);
  console.log(`  ├─ Requests:    ${counts.requests._count}`);
  console.log(`  ├─ Communities: ${counts.communities._count}`);
  console.log(`  ├─ Reviews:     ${counts.reviews._count}`);
  console.log(`  └─ Reports:     ${counts.reports._count}`);
  console.log();
  ...
  for (const [table, expectedCount] of Object.entries(expected)) {
    const actual = counts[table]?._count ?? 0;
    if (actual === expectedCount) {
      logSuccess(`${table}: ${actual} (expected ${expectedCount})`);
    } else {
      logError(`${table}: ${actual} (expected ${expectedCount})`);
      allGood = false;
    }
  }
...
async function main() {
  logHeader("🍎 Foodshare Database Seeder");
  ...
  if (isEmulator) {
    console.log(`  Emulator: ${BLUE}${EMULATOR_HOST}${RESET}`);
  }
  ...
  try {
    ...
    if (seedAll || args.includes("--categories-only")) {
      await seedCategories();
      await delay(200);
    }

    if (seedAll || args.includes("--users-only")) {
      await seedUsers();
      await delay(200);
    }
    ...
    if (!skipVerify) {
      ...
      if (verified) {
        ...
        logWarning("\n⚠️  Some tables have unexpected counts. Check the output above.");
      }
    }
